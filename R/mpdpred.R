#####################################################################
#### marginalized posterior density with flexible prediction sites
#####################################################################

## please keep the following comments in the code
...
mpdpred <- function(params, Y, X, k22mat, wmat, spcor, etype, ztype, retype,
                    weights, control)
################################################################################
# Y:        the concatenated sparse response Y = c(y, rep(0, nzp))
# X:        the concatenated sparse model matrix, (n + nzp) by (p + nzp)
# k22mat:   the design matrix of nonprediction spatial random effect, ny2 by nzu
