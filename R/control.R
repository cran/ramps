ramps.control <- function(iter = 1000, beta, sigma2.e, phi, sigma2.z, sigma2.re,
                          z.monitor = TRUE, file)
{
   if (!is.numeric(iter)) stop("MCMC iterations must be a numeric value or vector")
   iter <- as.integer(iter)
   iter <- if (length(iter) == 1) 1:iter
           else sort(unique(iter[iter > 0]))

   if (missing(beta)) beta <- param(NULL)
   else if (!is.param(beta)) stop("Incompatible data type for 'beta'")
   else if (beta$prior != "flat")
      stop("Only flat priors are supported for 'beta'")
   ...
   if (missing(sigma2.z)) sigma2.z <- param(NULL)
      ...
      stop("Only inverse gamma priors are supported for 'sigma2.z'")
   ...
   if (missing(sigma2.re)) sigma2.re <- param(NULL)
      ...
      stop("Only inverse gamma priors are supported for 'sigma2.re'")
                  ...
                  tuning = 1, monitor = TRUE, ...)
{
   ...
   switch(retval$prior,
      ...
      invgamma = {
         ...
         if (any(init <= 0)) stop("Initial values must be greater than 0")
      },
      normal = {
         ...
         if (class(val) == "try-error")
            stop("Normal variance hyperparameter must be positive definate")
   ...
   if (any(tuning <= 0)) stop("Tuninig parameters must be > 0")
   retval$tuning <- rep(tuning, length.out = n)

   retval$monitor <- as.logical(rep(monitor, length.out = n))
